...
// Allocate and fill in the sample_range_limit table 
{
	...
	for(j = 0; j < LEN256; j++)
		rlimit_table[j+256] = (BYTE) j;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	...
	for(j = 0; j < 128 ; j++)
		rlimit_table[j+1024] = (BYTE) j;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
}
...
void VQ_Initialize (struct COLOR_CACHE *VQ)
{
	...
	for (i = 0; i < 4; i++) {
		VQ->Index[i] = (unsigned char) i;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	}
	VQ->Color[0] = 0x008080;
	VQ->Color[1] = 0xFF8080;
	VQ->Color[2] = 0x808080;
