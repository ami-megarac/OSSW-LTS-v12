                                     ...
                                     int peci_fd, int retry)
{
    ...
    if (retry == 1)
    {
        if (ioctl(peci_fd, PECI_ISSUE_CMD_WITH_RETRIES, cmdPtr) != 0)
        {
            return PECI_CC_DRIVER_ERR;
        }
    }
    ...
    {
        if (ioctl(peci_fd, PECI_ISSUE_CMD, cmdPtr) != 0)
        {
            return PECI_CC_DRIVER_ERR;
        }
    }
...
#else
    ret = HW_peci_issue_cmd(PECI_CMD_GET_DIB, (char*)&cmd, peci_fd, false);
#endif
...
#else
    EPECIStatus ret = 
        HW_peci_issue_cmd(PECI_CMD_GET_TEMP, (char*)&cmd, peci_fd, false);
#endif
                                 ...
                                 uint8_t* pPkgConfig, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    PECICmd.target = target;
    PECICmd.read_len = u8ReadLen+PECI_RDPKGCFG_READ_LEN_BASE;
    PECICmd.write_len = PECI_RDPKGCFG_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_RDPKGCFG_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry
    PECICmd.write_buffer[Idx++] = u8Index;
    PECICmd.write_buffer[Idx++] = u16Value & 0xFF;
    PECICmd.write_buffer[Idx++] = (u16Value >> 8) & 0xFF;

    ret = HW_peci_issue_cmd(PECI_CMD_RD_PKG_CFG, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];
    if (ret == PECI_CC_SUCCESS)
    {
        memcpy(pPkgConfig, &(PECICmd.read_buffer[1]), u8ReadLen);
    }
#endif
...
#ifdef SPX_BMC_ACD
static unsigned char calculate_fcs(char* cmdPtr)
{
    unsigned char tbuf[12];

    memcpy(&tbuf[0], &(cmdPtr[2]), 12);

    return CalculateCRC8(tbuf, 12);
}
#endif
                                 ...
                                 uint8_t u8WriteLen, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    PECICmd.target = target;
    PECICmd.read_len = PECI_WRPKGCFG_READ_LEN;
    PECICmd.write_len = u8WriteLen+PECI_WRPKGCFG_WRITE_LEN_BASE;
    PECICmd.write_buffer[Idx++] = PECI_WRPKGCFG_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = u8Index;
    PECICmd.write_buffer[Idx++] = u16Param & 0xFF;
    PECICmd.write_buffer[Idx++] = (u16Param >> 8) & 0xFF;
    memcpy(&(PECICmd.write_buffer[Idx+4]), &u32Value, u8WriteLen);
    PECICmd.write_buffer[Idx++] = calculate_fcs((char*)&PECICmd) ^ 0x80;

    ret = HW_peci_issue_cmd(PECI_CMD_WR_PKG_CFG, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];
#endif
                         ...
                         uint64_t* u64MsrVal, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    PECICmd.target = target;
    PECICmd.read_len = PECI_RDIAMSR_READ_LEN;
    PECICmd.write_len = PECI_RDIAMSR_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_RDIAMSR_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = threadID;
    PECICmd.write_buffer[Idx++] = MSRAddress & 0xFF;
    PECICmd.write_buffer[Idx++] = (MSRAddress >> 8) & 0xFF;

    ret = HW_peci_issue_cmd(PECI_CMD_RD_IA_MSR, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];
    if (ret == PECI_CC_SUCCESS)
    {
        memcpy(u64MsrVal, &(PECICmd.read_buffer[1]), sizeof(uint64_t));
    }
#endif
...
#else
    ret = HW_peci_issue_cmd(PECI_CMD_RD_PCI_CFG, (char*)&cmd, peci_fd, true);
#endif
    *cc = cmd.cc;
                                      ...
                                      uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    address = u16Reg & 0x00000FFF;                  /* [11:0]  - Register */
    address |= (u8Fcn << 12) & 0x00007000; /* [14:12] - Function */
    address |= (u8Device << 15) & 0x000f8000;   /* [19:15] - Device   */
    address |= (u8Bus << 20) & 0x00F00000;      /* [23:20] - Bus      */
    ...
    PECICmd.target = target;
    PECICmd.read_len = u8ReadLen+PECI_RDPCICFGLOCAL_READ_LEN_BASE;
    PECICmd.write_len = PECI_RDPCICFGLOCAL_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_RDPCICFGLOCAL_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = address & 0xFF;
    PECICmd.write_buffer[Idx++] = (address >> 8) & 0xFF;
    PECICmd.write_buffer[Idx++] = (address >> 16) & 0xFF;

    ret = HW_peci_issue_cmd(PECI_CMD_RD_PCI_CFG_LOCAL, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];

    if (ret == PECI_CC_SUCCESS)
    {
        memcpy(pPCIReg, &(PECICmd.read_buffer[1]), u8ReadLen);
    }
#endif
...
#else
    ret = HW_peci_issue_cmd(PECI_CMD_WR_PCI_CFG_LOCAL, (char*)&cmd, peci_fd, true);
#endif
    *cc = cmd.cc;
    ...
    uint8_t* pPCIData, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    PECICmd.target = target;
    PECICmd.read_len = u8ReadLen+PECI_RDENDPTCFG_READ_LEN_BASE;
    PECICmd.write_len = PECI_RDENDPTCFG_PCI_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_RDENDPTCFG_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = u8MsgType;
    PECICmd.write_buffer[Idx++] = 0x0;
    PECICmd.write_buffer[Idx++] = 0x0;
    PECICmd.write_buffer[Idx++] = 0x0;
    PECICmd.write_buffer[Idx++] = PECI_ENDPTCFG_ADDR_TYPE_PCI;
    PECICmd.write_buffer[Idx++] = u8Seg;
    PECICmd.write_buffer[Idx++] = address & 0xFF;
    PECICmd.write_buffer[Idx++] = (address >> 8) & 0xFF;
    PECICmd.write_buffer[Idx++] = (address >> 16) & 0xFF;
    PECICmd.write_buffer[Idx++] = (address >> 24) & 0xFF;

    ret = HW_peci_issue_cmd(PECI_CMD_RD_END_PT_CFG, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];

    if (ret == PECI_CC_SUCCESS)
    {
        memcpy(pPCIData, &(PECICmd.read_buffer[1]), u8ReadLen);
    }
    else
    {
        ret = PECI_CC_DRIVER_ERR;
    ...
    uint8_t u8ReadLen, uint8_t* pMmioData, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    address = u8Fcn; /* [2:0] - Function */
    address |= u8Device << 3; /* [7:3] - Device */
	...
	PECICmd.write_len = PECI_RDENDPTCFG_MMIO_D_WRITE_LEN;
    else
	PECICmd.write_len = PECI_RDENDPTCFG_MMIO_Q_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_RDENDPTCFG_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = PECI_ENDPTCFG_TYPE_MMIO;
    PECICmd.write_buffer[Idx++] = 0x0;
    PECICmd.write_buffer[Idx++] = 0x0;
    PECICmd.write_buffer[Idx++] = u8Bar;
    PECICmd.write_buffer[Idx++] = u8AddrType;
    PECICmd.write_buffer[Idx++] = u8Seg;
    PECICmd.write_buffer[Idx++] = address & 0xFF;
    PECICmd.write_buffer[Idx++] = u8Bus;
    PECICmd.write_buffer[Idx++] = u64Offset & 0xFF;
    PECICmd.write_buffer[Idx++] = (u64Offset >> 8) & 0xFF;
    PECICmd.write_buffer[Idx++] = (u64Offset >> 16) & 0xFF;
    PECICmd.write_buffer[Idx++] = (u64Offset >> 24) & 0xFF;
    if (u8AddrType == PECI_ENDPTCFG_ADDR_TYPE_MMIO_Q)
    {
	PECICmd.write_buffer[Idx++] = (u64Offset >> 32) & 0xFF;
	PECICmd.write_buffer[Idx++] = (u64Offset >> 40) & 0xFF;
	PECICmd.write_buffer[Idx++] = (u64Offset >> 48) & 0xFF;
	PECICmd.write_buffer[Idx++] = (u64Offset >> 56) & 0xFF;
    }
    ...
    *cc = PECICmd.read_buffer[0];
    ...
    if (ret == PECI_CC_SUCCESS)
    {
        memcpy(pMmioData, &(PECICmd.read_buffer[1]), u8ReadLen);
    }
    else
    {
        ret = PECI_CC_DRIVER_ERR;
...
#else
    ret = HW_peci_issue_cmd(PECI_CMD_WR_END_PT_CFG, (char*)&cmd, peci_fd, true);
#endif
    *cc = cmd.cc;
                                     ...
                                     uint8_t* pData, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    PECICmd.target = target;
    PECICmd.read_len = u8ReadLen+PECI_CRASHDUMP_DISC_READ_LEN_BASE;
    PECICmd.write_len = PECI_CRASHDUMP_DISC_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_CRASHDUMP_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = PECI_CRASHDUMP_DISC_VERSION;
    PECICmd.write_buffer[Idx++] = PECI_CRASHDUMP_DISC_OPCODE;
    PECICmd.write_buffer[Idx++] = subopcode;
    PECICmd.write_buffer[Idx++] = param0;
    PECICmd.write_buffer[Idx++] = param1 & 0xFF;
    PECICmd.write_buffer[Idx++] = (param1 >> 8) & 0xFF;
    PECICmd.write_buffer[Idx++] = param2;

    ret = HW_peci_issue_cmd(PECI_CMD_CRASHDUMP_DISC, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];

    if (ret == PECI_CC_SUCCESS)
    {
        memcpy(pData, &(PECICmd.read_buffer[1]), u8ReadLen);
    }
    else
    {
        ret = PECI_CC_DRIVER_ERR;
                                    ...
                                    uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    PECICmd.target = target;
    PECICmd.read_len = u8ReadLen+PECI_CRASHDUMP_GET_FRAME_READ_LEN_BASE;
    PECICmd.write_len = PECI_CRASHDUMP_GET_FRAME_WRITE_LEN;
    PECICmd.write_buffer[Idx++] = PECI_CRASHDUMP_CMD;
    PECICmd.write_buffer[Idx++] = 0x0; //HostIDRetry;
    PECICmd.write_buffer[Idx++] = PECI_CRASHDUMP_GET_FRAME_VERSION;
    PECICmd.write_buffer[Idx++] = PECI_CRASHDUMP_GET_FRAME_OPCODE;
    PECICmd.write_buffer[Idx++] = param0 & 0xFF;
    PECICmd.write_buffer[Idx++] = (param0 >> 8) & 0xFF;
    PECICmd.write_buffer[Idx++] = param1 & 0xFF;
    PECICmd.write_buffer[Idx++] = (param1 >> 8) & 0xFF;
    PECICmd.write_buffer[Idx++] = param2;
    PECICmd.write_buffer[Idx++] = (param2 >> 8) & 0xFF;

    ret = HW_peci_issue_cmd(PECI_CMD_CRASHDUMP_GET_FRAME, (char*)&PECICmd, peci_fd, true);
    *cc = PECICmd.read_buffer[0];
                     ...
                     uint32_t respSize)
{
...
#else
    peci_cmd_t PECICmd;
    uint8_t u8RxBuf[PECI_DATA_BUF_SIZE] = {0};

    // Check for valid buffer sizes
    if (cmdSize > PECI_DATA_BUF_SIZE || respSize < u8ReadLen ||
        u8ReadLen > (PECI_DATA_BUF_SIZE - 1)) // response buffer is data + 1 status byte
    {
        peci_Close(peci_fd);
        return PECI_CC_INVALID_REQ;
    }
    ...
    PECICmd.target = target;
    PECICmd.read_len = u8ReadLen;
    PECICmd.write_len = (uint8_t)cmdSize;

    memcpy(&(PECICmd.write_buffer[0]), pRawCmd, cmdSize);
    memcpy(&(PECICmd.read_buffer[0]), u8RxBuf, PECI_DATA_BUF_SIZE);;

    ret = HW_peci_issue_cmd(PECI_CMD_XFER, (char*)&PECICmd, peci_fd, false);
#endif
