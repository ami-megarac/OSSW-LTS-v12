...
static int enable_mctp_pkt(void)
{
	...
	iowrite32 (MCTP_RX_COMPLETE | MCTP_TX_LAST | MCTP_RX_NO_CMD, ast_mctp->reg_base + AST_MCTP_IER);
...
{
	if(ast_mctp->rx_len == 0)
	{
		if ( wait_event_interruptible_timeout (ast_mctp->mctp_wq, (ast_mctp->flag & MCTP_EOM), msecs_to_jiffies(MCTP_PCIE_READ_TIMEOUT)) <= 0 )
		{
...
static void mctppcie_rx_combine_data(struct ast_mctp_info *ast_mctp)
{
	...
	while(ast_mctp->rx_cmd_desc[ast_mctp->rx_index].desc0 & CMD_UPDATE)
	{
		if(ast_mctp->rx_fifo_done != 1)
		{
			...
			//ast2500 : each 128 bytes align, and only 64 bytes can recevice
			memcpy(ast_mctp->rx_fifo + (64 * ast_mctp->rx_fifo_index), ast_mctp->rx_data + (ast_mctp->rx_index * 128), rx_len);
		...
		ast_mctp->rx_index++;
		ast_mctp->rx_index %= ast_mctp->rx_fifo_num;
	}
...
void ast_mctp_hardware_post_init(void)
{
	...
	for(i = 0; i < ast_mctp->rx_fifo_num; i++)
	{
		...
		ast_mctp->rx_cmd_desc[i].desc1 = RX_DATA_ADDR((ast_mctp->rx_data_dma + (ast_mctp->rx_fifo_size * i)));
...
void ast_mctp_hardware_init(void) 
{
	...
	//10K size memory
	//1st 4 KB : cmd desc --> 0 ~ 2048 : tx desc , 2049 ~ 4096 : rx desc
	//2nd 4 KB : tx data
	//3rd 16 KB: rx data --> 32 - 0x00 , 0x80 , 0x100, 0x180, each 128 align
	//4th 16 KB: rx data combine

	ast_mctp->mctp_dma = dma_alloc_coherent(NULL, 40*1024, &ast_mctp->mctp_dma_addr, GFP_KERNEL);
...
static int ast_scu_init(void)
{
    ...
    scu_irq = GetIrqFromDT("ami_scu", IRQ_SCU);

    ret = request_irq(scu_irq, ast_scu_isr, IRQF_SHARED, "ast-scu", &ret);
    if (ret) {
...
void ast_mctppcie_exit(void)
{
	...
	free_irq(mctp_irq, (void *)AST_MCTP_REG_BASE);
