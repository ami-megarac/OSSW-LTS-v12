...
u8 CalculateModulo100(u8 *Buff, int BuffLen) {
	...
	for (i = 0; i < BuffLen; i++)
		ChkSum += Buff[i];
	return (-ChkSum);/*Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /*2's compliment calculation changed from (-ChkSum) to (~ChkSum + 1) - Reason*/ 
}
...
int mousePkt(int buttonMask, int x, int y, int maxx, int maxy) {
	...
	pkt->ClientData = 0;
	pkt->Instance = 0;
	pkt->SeqNo = (u32) seqno++;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	pkt->Key = 0;
	...
	mouseRep.Event = (uint8) ((buttonMask == 4) ? 2 : buttonMask);/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	mouseRep.X = (uint16) X;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	mouseRep.Y = (uint16) Y;/* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
	mouseRep.Scroll = 0;
	memcpy(&pHIDPacket->Data, &mouseRep, MAX_MOUSE_REPORT_SIZE);  /* Fortify [Buffer Overflow]:: False Positive */ /* The number of bytes attempt to be copied will be limited to MAX_MOUSE_REPORT_SIZE - Reason*/
