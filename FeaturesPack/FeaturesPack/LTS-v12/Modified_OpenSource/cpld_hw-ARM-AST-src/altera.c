...
#define ALTERA_CPLD_NAME "ALTERA CPLD"
...
extern int jbcmain(char*,unsigned long *, unsigned long);//(int argc, char **argv);
...
const struct DEVICE_INFO device_list[]={
  ...
  {.idcode =ALTERA_CPLD_MAX_10_25DC_IDCODE, .name=ALTERA_CPLD_MAX_10_25DC_DEVNAME,.func = device_ALTERA, .support_loop_func = 0},
...
uint32_t hw_device_ctl(unsigned int cmd ,unsigned int *buf,unsigned long data_size){
  ...
  for(i = 0; i<sizeof(device_list)/sizeof(device_list[0]); i++){
    ...
    case IOCTL_JTAG_UPDATE_DEVICE:
    	...
    	ret = jbcmain((char*)"PROGRAM",(unsigned long *)buf, data_size);
      break;        

    case IOCTL_JTAG_VERIFY_JBC:
    	printk("IOCTL_JTAG_VERIFY_JBC...\n");
    	ret = jbcmain((char*)"VERIFY",(unsigned long *)buf, data_size);
		break;
    default:
    	ret = -1;
    }
...
uint32_t load_device_info(unsigned long id_code){
	...
	const JTAG_DEVICE_INFO Altera_information[]=
	{
		...
		{ //ALTERA_CPLD_MAX_10
		  .Device_Name    =               ALTERA_CPLD_MAX_10_25DC_DEVNAME,
		  .Device_ID      =               ALTERA_CPLD_MAX_10_25DC_IDCODE,
		  .Device_Column_Length   =       0,
		  .Device_Row_Length      =       0,
		  .Device_All_Bits_Length =       0,
		  .Device_Bscan_Length    =       0,
	...
	if (JTAG_device_information.Device_ID == 0xffffffff){
		if(Manufacturer_Identity(Altera_information[0].Device_ID ) ==Manufacturer_Identity(id_code)){
			...
			if(sizeof(JTAG_device_information.Device_Name)<sizeof(ALTERA_CPLD_NAME))
			{
    				printk("WARNING: there is a buffer overflows at JTAG_device_information.Device_Name\n");
    				ret=1;
			}

			count = snprintf((void*)&JTAG_device_information.Device_Name,sizeof(JTAG_device_information.Device_Name),"%s",ALTERA_CPLD_NAME);
			if(count!=(sizeof(ALTERA_CPLD_NAME)-1))
			{
     				printk("FAILED: Copy string of %s to JTAG_device_information.Device_Name is fail\n",ALTERA_CPLD_NAME);
     				ret=1;
			}
		}
	}
...
uint32_t set_device_sir (IO_ACCESS_DATA* code)
{
	SIR(SIR_BITS,(unsigned int) code->Data);
	code->Data = 0;
	if (code->Value !=0)
	  SDR(8* code->Value, 0,(void *) &code->Value);  
	code->Data = 0;
	return 0;
}
