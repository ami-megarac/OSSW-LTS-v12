...
if [ $featureenabled == 1 ]; then
	...
	if [ -f "/var/crashdump/json/bafi_decoded1.json" ]; then
		tar -czvf $crashdump_dir/bafi.1.gz -C /var/crashdump/json bafi_decoded1.json
	else
		if [ -f "$crashdump_dir/bafi.1.gz" ]; then
			rm $crashdump_dir/bafi.1.gz
		fi
	fi
	
	if [ -f "/var/crashdump/json/bafi_decoded2.json" ]; then
		tar -czvf $crashdump_dir/bafi.2.gz -C /var/crashdump/json bafi_decoded2.json
	else
		if [ -f "$crashdump_dir/bafi.2.gz" ]; then
			rm $crashdump_dir/bafi.2.gz
		fi
	fi
fi
