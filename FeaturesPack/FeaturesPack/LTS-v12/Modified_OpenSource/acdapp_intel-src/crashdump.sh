...
extract_existing_json()
{
	...
	if [ -f $crashdump_dir/sysdebug.1.gz ]; then
		tar -xzvf $crashdump_dir/sysdebug.1.gz -C /var/crashdump/json/
	fi
	
	if [ -f $crashdump_dir/sysdebug.2.gz ]; then
		tar -xzvf $crashdump_dir/sysdebug.2.gz -C /var/crashdump/json
	fi
}

extract_existing_bafi_json()
{
	isFeatureEnabled CONFIG_SPX_FEATURE_ACD_BAFI_SUPPORT
	featureenabled=$?
	if [ $featureenabled == 1 ]; then
		#extract compressed BAFI JSON files at boot time
		if [ ! -d "/var/crashdump/json" ]; then
			mkdir -p /var/crashdump/json
		fi
	
		if [ -f $crashdump_dir/bafi.1.gz ]; then
			tar -xzvf $crashdump_dir/bafi.1.gz -C /var/crashdump/json/
		fi
	
		if [ -f $crashdump_dir/bafi.2.gz ]; then
			tar -xzvf $crashdump_dir/bafi.2.gz -C /var/crashdump/json
		fi
	fi
}
