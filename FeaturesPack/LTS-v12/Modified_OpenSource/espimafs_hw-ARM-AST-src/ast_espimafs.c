...
int ast_espimafs_init(void)
{
    ...
    *(volatile u32 *)(IO_ADDRESS(AST_SCU_REG_BASE)) = 0;            //Lock SCU register
    ...
    espimafs_irq = GetIrqFromDT("ami_espi", AST_ESPI_IRQ);
    ret = request_irq(espimafs_irq, ast_espimafs_handler, IRQF_SHARED, AST_ESPIMAFS_DRIVER_NAME, ast_espimafs_virt_base + AST_ESPI_FLASH_CHAN_ID);
    if (ret) {
...
void ast_espimafs_exit(void)
{
    ...
    free_irq(espimafs_irq, ast_espimafs_virt_base + AST_ESPI_FLASH_CHAN_ID);
