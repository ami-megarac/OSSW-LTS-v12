...
#define AST_JTAG_BUFFER_SIZE 0x180000 //1.5MB
#define AST_FW_BUFFER_SIZE  0x180000  //1.5MB
...
static jtag_debug_hw_operations_t *pjtag_debug_ops = NULL;
...
int register_jtag_debug_hw_device_ops(jtag_debug_hw_operations_t *pjDebugHwd)
{
	pjtag_debug_ops = pjDebugHwd;
	return 0;
}

int unregister_jtag_debug_hw_device_ops (void)
{
	if(pjtag_debug_ops != NULL){
		pjtag_debug_ops = NULL;
	}
...
static long is_normal_cmd(unsigned int cmd)
{
	switch (cmd)
	{
		case  IOCTL_IO_READ:                           
		case  IOCTL_IO_WRITE:                            
		case  IOCTL_REMAP:                  
		case  IOCTL_REAL_IO_READ:             
		case  IOCTL_REAL_IO_WRITE:           
		case  IOCTL_BIT_STREAM_BASE:             
		case  IOCTL_TX_BIT_STREAM:              
		case  IOCTL_GET_SOCKET:              
        ...
        case  IOCTL_JTAG_SIR:
			//printk("this is normal jtag command\n");
			return 1;
		default:
			break;
	}
...
static long is_jtag_debug_cmd(unsigned int cmd)
{	
	switch (cmd)
	{
		case ASPEED_JTAG_IOCRUNTEST:
		case ASPEED_JTAG_XFER:
		...
		//printk("this is jtag amd command\n");
			return 1;
		default:
			break;
	}
...
static long normal_jtag_ioctl(struct file *file,unsigned int cmd, unsigned long arg)
{
	struct jtag_dev *pdev = (struct jtag_dev*) file->private_data;
	...
	switch (cmd)
	{
		...
		case IOCTL_JTAG_UPDATE_JBC:
			...
			*(IO_ACCESS_DATA *)arg = Kernal_IO_Data;
			break;
       case IOCTL_JTAG_SIR:
			ret=pjhwd_ops->set_hw_device_sir(&Kernal_IO_Data);
			if(ret!=0) ret=-1;
			*(IO_ACCESS_DATA *)arg = Kernal_IO_Data;
			break;
...
{
	int ret=0;
	if(is_normal_cmd(cmd))
	{	
		normal_jtag_ioctl(file,cmd,arg);
	}
	else if(is_jtag_debug_cmd(cmd)) //For jtag Debug control
	{	
		if(pjtag_debug_ops)
			pjtag_debug_ops->jtag_debug_ioctl(cmd,arg);
	}
	else
	{
		printk ( "Invalid JTAG Function\n");
...
EXPORT_SYMBOL(register_jtag_debug_hw_device_ops);
EXPORT_SYMBOL(unregister_jtag_debug_hw_device_ops);
