...
typedef enum jtag_xfer_type {
	JTAG_DR_SCAN = 0,
	JTAG_IR_SCAN
} xfer_type;

typedef enum jtag_tap_state {
	JTAG_TAP_STATE_TLRESET,
	JTAG_TAP_STATE_IDLE,
	JTAG_TAP_STATE_SELECTDR,
	JTAG_TAP_STATE_CAPTUREDR,
	JTAG_TAP_STATE_SHIFTDR,
	JTAG_TAP_STATE_EXIT1DR,
	JTAG_TAP_STATE_PAUSEDR,
	JTAG_TAP_STATE_EXIT2DR,
	JTAG_TAP_STATE_UPDATEDR,
	JTAG_TAP_STATE_SELECTIR,
	JTAG_TAP_STATE_CAPTUREIR,
	JTAG_TAP_STATE_SHIFTIR,
	JTAG_TAP_STATE_EXIT1IR,
	JTAG_TAP_STATE_PAUSEIR,
	JTAG_TAP_STATE_EXIT2IR,
	JTAG_TAP_STATE_UPDATEIR	
} tap_state;
...
struct scan_xfer {
    xfer_mode     mode;			//0 :HW mode, 1: SW mode    
    unsigned char direct;		//1 ; read , 2 : write, 3: write + read     
    unsigned int length;		//bits
    unsigned int *tdio;
    unsigned char enddr;        
    unsigned char type;			//0: dr_scan, 1: ir_scan
};
...
#define  ASPEED_JTAG_IOCRUNTEST             _IOW('j', 24, struct runtest_idle)
#define  ASPEED_JTAG_XFER                 	_IOWR('j',25, struct scan_xfer)
