...
int DecodeCheck(char *outbuf, char *string, int outlen)
{
    ...
    if ( 0 == strlen(string) ) {	/* Fortify [String Termination Error]:: False Positive */
	/* Reason for False Positive - buffer string is terminated with NULL character */ 
        *outbuf = '\0'; 
        return 0; 
    } 
...
int Decode64(char *outbuf, char *string, int outlen)
{
    ...
    ret = cyoBase64Decode(outbuf, string, strlen(string));	/* Fortify [String Termination Error]:: False Positive */
	/* Reason for False Positive - buffer string is terminated with NULL character */
    outbuf[outlen-1] = '\0';
...
int Decode64Binary(char *outbuf, char *string, int outlen)
{
    ...
    ret = cyoBase64DecodeBinary(outbuf, string, strlen(string));	/* Fortify [String Termination Error]:: False Positive */
		/* Reason for False Positive - buffer string is terminated with NULL character */
...
void ConvertHexBinarytoStr(char *hex, char *str,int len)
{
	...
	/* Reason for False Positive - destination has the sufficient memory to store the data */
    }
}
