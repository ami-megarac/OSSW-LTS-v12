...
Usb_OsInitDPC(DPC UsbDeferProcess)
{
	...
	return wait_for_completion_interruptible(&DPC_Init); /* Return values for fortify [Dead Code].*/
}
...
int SendEthernetPacket(struct sk_buff *skb, struct net_device *dev)
{
	int r;
	int len;
	unsigned char *data, shortpkt[ETH_ZLEN]; /* Fix for fortify [Type Mismatch: Signed to Unsigned].*/
#if (LINUX_VERSION_CODE < KERNEL_VERSION (2,6,30))
	...
	if(r == NUM_TX_BUFS)
	{
		TDBG("ERROR: did not get TX buffer and so dropping\n");
...
int Net_OSPushPacket(NET_DEVICE *Dev, uint8 *Data, int Len)
{
	...
	switch(netif_rx(skb))
	{
		...
		case NET_RX_DROP:
			RX_Stats->rx_dropped++;
			return 0; // it is ok, need not to raise an error. // Known case: usb0 is not up but disable power save mode or launch USB related functions(KVM/media redirection).
