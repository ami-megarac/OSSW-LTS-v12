...
int pilot_unregsensorints (  void* gpin_data)
{
    ...
    for (i = 0; i < m_total_intr_sensors; i++)
    {
        if (m_intr_sensors[i].gpio_number == ((Gpio_data_t*)gpin_data)->PinNum)
        {
            --m_total_intr_sensors;
            if (i < m_total_intr_sensors)
            {
                memcpy(&m_intr_sensors[i], &m_intr_sensors[i + 1], sizeof(gpio_interrupt_sensor) * (m_total_intr_sensors - i));
            }
            break;
        }
    }
