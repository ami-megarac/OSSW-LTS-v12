--- uboot/arch/arm/cpu/armv7/pilot4/ddr_options.c	1970-01-01 08:00:00.000000000 +0800
+++ mywork/arch/arm/cpu/armv7/pilot4/ddr_options.c	2018-02-12 16:10:03.041418757 +0800
@@ -0,0 +1,5406 @@
+/*******************************************************************************
+ *
...
+#else
+
+#define  BRCL_VERSION 0x4157
+// ASIC SIM ddr init
+//tU32 ddrinit(tU8 ddr_type){
+
+#if defined (CHECK_MRS) 
+void copy_mrs(volatile unsigned int* srcAddr, volatile unsigned int* dstAddr)
+{
+    asm volatile ("push    {r0,r1,r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, lr}");
+    asm volatile(
+            "mov r0, %0\n\t"
+            "mov r1, %1\n\t"
+            :
+            : "r" (srcAddr), "r" (dstAddr));
+    asm volatile ("  LDMIA  r0, {R2-R5} ");
+    asm volatile ("  STMIA  r1!, {R2-R5} ");
+    asm volatile ("pop {r0,r1,r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, pc}");
+}
+
+//extern void copy_mrs(unsigned long src, unsigned long dst);
+#endif 
+
+tU32 ddrinit( void )
+{
...
+  *(tPVU32)(MC_REG_ADDRESS18) = (*(tPVU32)(MC_REG_ADDRESS18) & 0xffffdfff);
+  //MR6 A7=1'b1 [VrefDQ Training Enable]
+  *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x0480d404;
+  delay(75);//wait for tVREFDQE
+  //MR6 A6=1'b0, A5-0=5'h17 [Vref Range1, 74.95%]
+  *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x0497d404;
+  delay(75);//Vref_time-Long
+  //MR6 A7=1'b0 [VrefDQ Training Exit]
...
+#if defined (CHECK_MRS) 
+
+    *(tPVU32)(MC_REG_ADDRESS18) = (*(tPVU32)(MC_REG_ADDRESS18) & ~0x00003000) | 0x00001000; //Disable refresh and enable reset refresh
+  DELAY(5);
+    *(tPVU32)(MC_REG_ADDRESS18) = (*(tPVU32)(MC_REG_ADDRESS18) & ~0x00003000) | 0x00000000; // cleared reset refresh bit
+  DELAY(5);
+    *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0x00000800) | 0x00000800; //Precharge all
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x800) != 0);
+
+  DELAY(5);
+    *(tPVU32)(MC_REG_ADDRESS10) = (*(tPVU32)(MC_REG_ADDRESS10) & ~0x00000400) | 0x00000400; //enabled MPR burst 8 read
+    #if defined (MPR_PRNT)
+	#if defined DEFAULT_PRINTS
+	  putc('S');  putc('t');  putc('a');  putc('r');  putc('t');  putc('e');  putc('d');  putc(' ');  putc('M');  putc('R');  putc('S');  putc(' ');  putc('R');  putc('e');  putc('a');  
+	  putc('d');  putc(' ');  putc('t');  putc('h');  putc('r');  putc('o');  putc('u');  putc('g');  putc('h');  putc(' ');  putc('M');  putc('R');  putc('S');  putc('3');  putc(' ');  
+	  putc('c');  putc('o');  putc('m');  putc('m');  putc('a');  putc('n');  putc('d');  putc(' ');  putc('\n'); putc('\n'); putc('\r'); 
+	#endif
+	#if defined (HW_ENVPRN)
+       nc_printf("Started MRS Read through MRS3 command \n");
+    #endif
+    #endif
+
+    #if defined (MPR_PRNT)
+       rd_data = *(tPVU32)(MC_REG_ADDRESS00);
+	#if defined DEFAULT_PRINTS
+	  putc('R');  putc('e');  putc('a');  putc('d');putc(' '); putc('d');  putc('a');  putc('t');  putc('a');  putc(' ');  putc('b'); putc('e'); putc('f'); putc('o'); putc('r'); 
+	  putc('e');  putc(' ');  putc('B');  putc('L');putc('8'); putc(' ');  putc('e');  putc('n');  putc('a');  putc('b');  putc('l'); putc('i'); putc('n'); putc('g'); putc(' '); 
+	  putc('i');  putc('n');  putc(' ');  putc('M');putc('R'); putc('S');  putc('0');  putc(' ');  putc('i');  putc('s');  putc(' '); print_reg(rd_data);   putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+       nc_printf("read data before BL8 enabling in MRS0 is %x\n", rd_data);
+    #endif
+    #endif
+
+    *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0x0003ffc7) | 0x00001404; //MRS0. BL8 fixed for MPR Reads
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+    #if defined (MPR_PRNT)
+       rd_data = *(tPVU32)(MC_REG_ADDRESS00);
+	#if defined DEFAULT_PRINTS
+	  putc('R');  putc('e');  putc('a');  putc('d');  putc(' ');  putc('d');  putc('a');  putc('t');  putc('a');  putc(' ');  putc('a'); putc('f'); putc('t'); putc('e'); putc('r'); 
+	  putc(' ');  putc('B');  putc('L');  putc('8');  putc(' ');  putc('e');  putc('n');  putc('a');  putc('b');  putc('l');  putc('i'); putc('n'); putc('g'); putc(' '); 
+	  putc('a');  putc('n');  putc('d');  putc(' ');  putc('b');  putc('e');  putc('f');  putc('o');  putc('r');  putc('e');  putc(' '); putc('M'); putc('P'); putc('R');  putc(' '); 
+	  putc('e');  putc('n');  putc('a');  putc('b');  putc('l');  putc('i');  putc('n');  putc('g');  putc(' ');  putc('i');  putc('s'); putc(' '); print_reg(rd_data);  putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+       nc_printf("read data after BL8 enabling and before MPR enabling is %x\n", rd_data);
+    #endif
+    #endif
+
+    *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x00067404; //MRS3. Enabled, MPR Page2
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+  delay(200);
+    #if defined (MPR_PRNT)
+       rd_data = *(tPVU32)(MC_REG_ADDRESS00);
+	#if defined DEFAULT_PRINTS
+	  putc('R');  putc('e');  putc('a');  putc('d');  putc(' '); putc('d');  putc('a'); putc('t');  putc('a');  putc(' ');  putc('a');  putc('f');  putc('t'); putc('e');  putc('r'); 
+	  putc(' ');  putc('e');  putc('n');  putc('a');  putc('b'); putc('l');  putc('i'); putc('n');  putc('g');  putc(' ');  putc('M');  putc('P');  putc('R'); putc(' ');  
+	  putc('r');  putc('e');  putc('a');  putc('d');  putc(' '); putc('i');  putc('n'); putc(' ');  putc('M');  putc('R');  putc('S');  putc('3');  putc(' ');  putc('i');  putc('s');
+	  putc(' ');  print_reg(rd_data);     putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+       nc_printf("read data after enabling MPR read in MRS3 is %x\n", rd_data);
+        nc_printf("\n");
+    #endif
+    #endif
+
+  DELAY(5);
+
+//Below 4 lines of code should be uncommented for RTL SIM and comment out code from copy_mrs to #endif before DELAY(5)
+        //rd_data_0 = *(tPVU32)0x84000000;
+        //#if defined (MPR_PRNT)
+        //    nc_printf("MPR rd_data_0 is %x for Page 2 MPR with BA as 0 of MRS3\n", rd_data_0);
+        //#endif
+        copy_mrs((tPVU32)(0x84000000), &mrs_values[0]);
+    #if defined (MPR_PRNT)
+        //rd_data = (|(mrs_values[0]&0xFFFF0000)<<5)| (|(mrs_values[1]&0xFFFF)<<4)|(|(mrs_values[1]&0xFFFF0000)<<3)|(|(mrs_values[2]&0xFFFF)<<2)|(|(mrs_values[2]&0xFFFF0000)<<1)|(|(mrs_values[3]&0xFFFF)<<0);
+        A0 = ((mrs_values[0]&0xFFFF0000)>>16);
+        A1 = ((mrs_values[1]&0xFFFF));
+        A2 = ((mrs_values[1]&0xFFFF0000)>>16);
+        A3 = ((mrs_values[2]&0xFFFF));
+        A4 = ((mrs_values[2]&0xFFFF0000)>>16);
+        A5 = ((mrs_values[3]&0xFFFF));
+        rd_data = (A0&0x20)|(A1&0x10) | (A2&0x8) | (A3 & 0x4) | (A4 & 0x2) | (A5 & 0x1);
+
+	#if defined DEFAULT_PRINTS
+	  putc('M');  putc('P');  putc('R');  putc(' ');  putc('P');  putc('a');  putc('g');  putc('e');  putc(' ');  putc('2');  putc(' ');  putc('M'); putc('P');  putc('R'); putc(' '); 
+	  putc('w');  putc('i');  putc('t');  putc('h');  putc(' ');  putc('B');  putc('A');  putc(' ');  putc('a');  putc('s');  putc(' ');  putc('0'); putc(' ');  putc('M'); putc('R'); 
+	  putc('S');  putc('3');  putc(':');  putc(' ');  print_reg(rd_data);     putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+        nc_printf("MPR Page 2 MPR with BA as 0 of MRS3:%x\n",rd_data);
+        //nc_printf("MPR 1st 32bit is %x for Page 2 MPR with BA as 0 of MRS3\n", mrs_values[0]);
+        //nc_printf("MPR 2nd 32bit  is %x for Page 2 MPR with BA as 0 of MRS3\n", mrs_values[1]);
+        //nc_printf("MPR 3rd 32bit  is %x for Page 2 MPR with BA as 0 of MRS3\n", mrs_values[2]);
+        //nc_printf("MPR 4th 32bit  is %x for Page 2 MPR with BA as 0 of MRS3\n", mrs_values[3]);
+        nc_printf("\n");
+    #endif
+    #endif
+  DELAY(5);
+//Below 4 lines of code should be uncommented for RTL SIM and comment out code from copy_mrs to #endif before DELAY(5)
+       //rd_data_0 = *(tPVU32)0x84001000;
+       //#if defined (MPR_PRNT)
+       //    nc_printf("MPR rd_data_0 is %x for Page 2 MPR with BA as 1 of MRS3\n", rd_data_0);
+       //#endif
+        copy_mrs((tPVU32)(0x84001000), &mrs_values[0]);
+    #if defined (MPR_PRNT)
+        A0 = ((mrs_values[0]&0xFFFF0000)>>16);
+        A1 = ((mrs_values[1]&0xFFFF));
+        A2 = ((mrs_values[1]&0xFFFF0000)>>16);
...
+	#if defined DEFAULT_PRINTS
+	  putc('M');  putc('P');  putc('R');  putc(' ');  putc('P');  putc('a');  putc('g');  putc('e');  putc(' ');  putc('2');  putc(' ');  putc('M'); putc('P'); putc('R'); putc(' '); 
+	  putc('w');  putc('i');  putc('t');  putc('h');  putc(' ');  putc('B');  putc('A');  putc(' ');  putc('a');  putc('s');  putc(' ');  putc('1'); putc(' '); putc('o'); putc('f'); 
+	  putc(' ');  putc('M');  putc('R');  putc('S');  putc('3');  putc(':');  putc(' ');  print_reg(rd_data); putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+        nc_printf("MPR Page 2 MPR with BA as 1 of MRS3:%x\n",rd_data);
+        //nc_printf("MPR 1st 32bit is %x for Page 2 MPR with BA as 1 of MRS3\n", mrs_values[0]);
+        //nc_printf("MPR 2nd 32bit  is %x for Page 2 MPR with BA as 1 of MRS3\n", mrs_values[1]);
+        //nc_printf("MPR 3rd 32bit  is %x for Page 2 MPR with BA as 1 of MRS3\n", mrs_values[2]);
+        //nc_printf("MPR 4th 32bit  is %x for Page 2 MPR with BA as 1 of MRS3\n", mrs_values[3]);
+        nc_printf("\n");
+    #endif
+    #endif
+
+  DELAY(5);
+//Below 4 lines of code should be uncommented for RTL SIM and comment out code from copy_mrs to #endif before DELAY(5)
+        //rd_data_0 = *(tPVU32)0x84002000;
+        //#if defined (HW_ENVPRN)
+        //    nc_printf("MPR rd_data_0 is %x for Page 2 MPR with BA as 2 of MRS3\n", rd_data_0);
+        //#endif
+        copy_mrs((tPVU32)(0x84002000), &mrs_values[0]);
+    #if defined (MPR_PRNT)
+        A0 = ((mrs_values[0]&0xFFFF0000)>>16);
+        A1 = ((mrs_values[1]&0xFFFF));
+        A2 = ((mrs_values[1]&0xFFFF0000)>>16);
...
+	#if defined DEFAULT_PRINTS
+	  putc('M');  putc('P');  putc('R');  putc(' ');  putc('P');  putc('a');  putc('g');  putc('e');  putc(' ');  putc('2');  putc(' ');  putc('M'); putc('P'); putc('R'); putc(' '); 
+	  putc('w');  putc('i');  putc('t');  putc('h');  putc(' ');  putc('B');  putc('A');  putc(' ');  putc('a');  putc('s');  putc(' ');  putc('2'); putc(' '); putc('o'); putc('f'); 
+	  putc(' ');  putc('M');  putc('R');  putc('S');  putc('3');  putc(':');  putc(' ');  print_reg(rd_data);     putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+        nc_printf("MPR Page 2 MPR with BA as 2 of MRS3:%x\n",rd_data);
+        //nc_printf("MPR 1st 32bit is %x for Page 2 MPR with BA as 2 of MRS3\n", mrs_values[0]);
+        //nc_printf("MPR 2nd 32bit  is %x for Page 2 MPR with BA as 2 of MRS3\n", mrs_values[1]);
+        //nc_printf("MPR 3rd 32bit  is %x for Page 2 MPR with BA as 2 of MRS3\n", mrs_values[2]);
+        //nc_printf("MPR 4th 32bit  is %x for Page 2 MPR with BA as 2 of MRS3\n", mrs_values[3]);
+        nc_printf("\n");
+    #endif
+    #endif
+
+  DELAY(5);
+//Below 4 lines of code should be uncommented for RTL SIM and comment out code from copy_mrs to #endif before MC_REG_ADDRESS10 access 
+       //rd_data_0 = *(tPVU32)0x84003000;
+       //#if defined (HW_ENVPRN)
+       //    nc_printf("MPR rd_data_0 is %x for Page 2 MPR with BA as 3 of MRS3\n", rd_data_0);
+       //#endif
+        copy_mrs((tPVU32)(0x84003000), &mrs_values[0]);
+    #if defined (MPR_PRNT)
+        A0 = ((mrs_values[0]&0xFFFF0000)>>16);
+        A1 = ((mrs_values[1]&0xFFFF));
+        A2 = ((mrs_values[1]&0xFFFF0000)>>16);
...
+	#if defined DEFAULT_PRINTS
+	  putc('M');  putc('P');  putc('R');  putc(' ');  putc('P');  putc('a');  putc('g');  putc('e');  putc(' ');  putc('2');  putc(' ');  putc('M'); putc('P'); putc('R'); putc(' '); 
+	  putc('w');  putc('i');  putc('t');  putc('h');  putc(' ');  putc('B');  putc('A');  putc(' ');  putc('a');  putc('s');  putc(' ');  putc('3'); putc(' '); putc('o'); putc('f'); 
+	  putc(' ');  putc('M');  putc('R');  putc('S');  putc('3');  putc(':');  putc(' ');  print_reg(rd_data);     putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+        nc_printf("MPR Page 2 MPR with BA as 3 of MRS3:%x\n",rd_data);
+        //nc_printf("MPR 1st 32bit is %x for Page 2 MPR with BA as 3 of MRS3\n", mrs_values[0]);
+        //nc_printf("MPR 2nd 32bit  is %x for Page 2 MPR with BA as 3 of MRS3\n", mrs_values[1]);
+        //nc_printf("MPR 3rd 32bit  is %x for Page 2 MPR with BA as 3 of MRS3\n", mrs_values[2]);
+        //nc_printf("MPR 4th 32bit  is %x for Page 2 MPR with BA as 3 of MRS3\n", mrs_values[3]);
+        nc_printf("\n");
+    #endif
+    #endif
+
+    *(tPVU32)(MC_REG_ADDRESS10) = (*(tPVU32)(MC_REG_ADDRESS10) & ~0x00000400) | 0x00000000; //Disabled enabled MPR burst 8 read
+
+    *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0x0007ffc7) | 0x00007404; //MRS3. Memory Flow Enabled
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+    #if defined (MPR_PRNT)
+       rd_data = *(tPVU32)(MC_REG_ADDRESS00);
+	#if defined DEFAULT_PRINTS
+	  putc('R');  putc('e');  putc('a');  putc('d');  putc(' '); putc('d');  putc('a'); putc('t');  putc('a');  putc(' ');  putc('a');  putc('f');  putc('t'); putc('e');  putc('r'); 
+	  putc(' ');  putc('e');  putc('n');  putc('a');  putc('b'); putc('l');  putc('i'); putc('n');  putc('g');  putc(' ');  putc('M');  putc('e');  putc('m'); putc('o');  putc('r');
+	  putc('y');  putc(' ');  putc('r');  putc('e');  putc('a'); putc('d');  putc(' '); putc('i');  putc('n');  putc(' ');  putc('M');  putc('R');  putc('S'); putc('3');  putc(' ');
+	  putc('i');  putc('s');  putc(' ');  print_reg(rd_data);    putc('\n'); 
+	#endif
+	#if defined (HW_ENVPRN)
+       nc_printf("read data after enabling Memory read in MRS3 is %x\n", rd_data);
+    #endif
+    #endif
+
+if(DLL_OFF==1) {
...
+     while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+     } else {//#else
+     *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x03151404; //MRS0. BC4/BL8 OTF
+     while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+    }
+}
+     #if defined (MPR_PRNT)
...
+	#if defined DEFAULT_PRINTS
+	  putc('R');  putc('e');  putc('a');  putc('d');  putc(' ');  putc('d');  putc('a');  putc('t');  putc('a');  putc(' ');  putc('a');  putc('f'); putc('t'); putc('e'); putc('r'); 
+	  putc(' ');  putc('r');  putc('e');  putc('v');  putc('e');  putc('r');  putc('t');  putc('i');  putc('n');  putc('g');  putc(' ');  putc('t'); putc('o'); putc(' '); putc('B');  
+	  putc('C');  putc('4');  putc('/');  putc('B');  putc('L');  putc('8');  putc(' ');  putc('O');  putc('T');  putc('F');  putc(' '); 
+	  putc('M');  putc('e');  putc('m');  putc('o');  putc('r');  putc('y');  putc(' ');  putc('r');  putc('e');  putc('a');  putc('d');  putc(' '); putc('i'); putc('n'); putc(' ');  
+	  putc('M');  putc('R');  putc('S');  putc('0');  putc(' ');  putc('i');  putc('s');  putc(' ');  print_reg(rd_data);     
+	#endif
+	#if defined (HW_ENVPRN)
+       nc_printf("read data after reverting to BC4/BL8 OTF Memory read in MRS0 is %x\n", rd_data);
+    #endif
+    #endif
+
+  DELAY(5);
+    *(tPVU32)(MC_REG_ADDRESS18) = (*(tPVU32)(MC_REG_ADDRESS18) & ~0x00003000) | 0x00002000; // Enabled Refresh 
+
+#endif
+  //shutoff mcss register clk
