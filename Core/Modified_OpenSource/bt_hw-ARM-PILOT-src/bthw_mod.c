...
#define BT_TIMEOUT              15
#define BT_RETRY                5000
...
bthw_write_bt_data_buf (u8 Ch, u8* Buf, u32 Len)
{
	...
	if (Ch >= BT_CHANNEL_NUM) 
	{
        ...
        reg = bt_read_reg (BTCR);
	while ((retry > 0) && (reg & H_BUSY))
	{
 		TDBG ("[W]Host is busy, going to retry [ %d ]\n", retry);
		retry--;
		udelay(BT_TIMEOUT);
		reg = bt_read_reg (BTCR);
	}
	...
	if(retry <= 0)
	{
		TCRIT ("[W] Host is always Busy \n");
	}
	else { retry = BT_RETRY; }
	
	TDBG( "[W] Clear Write PTR \n");
	clr_write_fifo_ptr();
        if(count == 0 )
	{
		bt_write_reg ((uint8_t)Len, BTDR);
                count++;
        }
	while (Len)
	{
		...
		Len -= 1;
                ...
                if(count % BT_BUFFER_SIZE == 0)
		{
		    TDBG("[W]Breaking loop to send one set of %d packets\n", BT_BUFFER_SIZE);
		    break;
		}
	}

	TDBG ("[W]Setting the B2H attention bit\n");
	set_b2h_attention ();
	
	if(Len != 0)
	{
		TDBG("[W]Length Remaining to be sent - %d\n",Len);
...
{
	volatile uint8_t Len = 0, count = 0, reg = 0;
        int retry = BT_RETRY;
	if (Ch >= BT_CHANNEL_NUM) 
	{ 
		TCRIT ("Invalid BT Channel Number\n"); 
	...
	{ 
		*Buf = bt_read_reg (BTDR); 
		...
		Buf += 1;
	        ...
	        if(count % BT_BUFFER_SIZE == 0)
		{
                    mdelay(5); /* Workaround: Setting BMC_BUSY get missed out 
                                  due to time delay, so added time delay to make it work.
                               */
                    clr_h2b_attention ();
                    clr_bmc_busy ();
                    retry = BT_RETRY;
		    reg = bt_read_reg (BTCR);
		    while ((retry > 0) && (!(reg & H2B_ATN)))
		    {
				TDBG("[R]Waiting for H2B attention bit, retrying [ %d] reg = %x \n", retry, reg);
				retry--;
				udelay(BT_TIMEOUT);
				reg = bt_read_reg (BTCR);
		    }
		    ...
		    {
				reg = bt_read_reg (BTCR);
                                TCRIT("Ctr Reg value :%x",reg);
				TCRIT("[R] Polling for H2B_ATN Failed \n");
		    } 
			retry = BT_RETRY; 
			TDBG("[R]Setting BMC busy bit and Clearing H2B attention \n");
			set_bmc_busy ();
			
	         
		}
         } 
    TDBG("[R]Clearing BMC busy bit\n");
    clr_h2b_attention (); 
    clr_bmc_busy (); 
