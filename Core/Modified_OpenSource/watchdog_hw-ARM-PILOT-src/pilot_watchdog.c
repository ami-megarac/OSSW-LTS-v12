 ...
 *The watchdog counter will always reloads with the SYSWCFR3[15:0] after expires, and watchdog 
...
static void pilot_watchdog_set_value(int value)
{
        ...
        else
        {
                if((current_wdt_device > MIN_WDT_DEVICE) && (current_wdt_device <= MAX_WDT_DEVICES))
                {
                        ...
                        iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCFR);
                }
        }
#endif
}
...
static void pilot_watchdog_count(void)
{
        ...
        else
        {
                if((current_wdt_device > MIN_WDT_DEVICE) && (current_wdt_device <= MAX_WDT_DEVICES))
                {
                        reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCR);
                        reg|= PILOT_WATCHDOG_WDCTL_TRIGGER_NEW_WDT;
                        iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCR);
                }
        }
#endif  
}
...
static void pilot_watchdog_enable(void)
{
        ...
        /* disable WDT1 of u-boot , before enabling WDT3, So that U-boot can differentiate between watchdog causing devises.*/
        ...
        else
        {
                if((current_wdt_device > MIN_WDT_DEVICE) && (current_wdt_device <= MAX_WDT_DEVICES))
                {
                        reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + SYSWCR);
                        reg &= ~PILOT_WATCHDOG_RESET_ENABLE;
                        iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + SYSWCR);
                        reg = 0;
                        iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + SYSWRERL);
                        iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + SYSWRERH);
                }
        }
#endif
 
        /* Enable WDT3 for fail safe booting.*/
        if (current_wdt_device == 1)
        {
                reg = ioread32((void __iomem*)pilot_watchdog_virt_base + PILOT_WATCHDOG_WDCTL);
        ...
        else
        {
                if((current_wdt_device > MIN_WDT_DEVICE) && (current_wdt_device <= MAX_WDT_DEVICES))
                {
                    ...
                    /* Clear the SYSRST_STATUS bit 2 to make sure the status bits are cleared */
                    reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + SYSRST_STATUS);
                    reg |=(1 << (current_wdt_device - 1));
                    iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + SYSRST_STATUS);             
                   
                    /* Set the watchdog timer in stopped state by setting SYSWCR3[0]= 0 */
                    reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCR);
                    reg  &=  ~PILOT_WATCHDOG_RESET_ENABLE;
                    iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCR);
                    
                    /*Program the count value in register SYSWCFR3[15:0]*/
                    // Watchdog timer counter value is already set in "pilot_watchdog_set_value" function        

                    /* select modules which need to be reset from the registers SYSWRERL3 and SYSWRERH3 */
                    ...
                    reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWRERL);
                    reg = g_reset_enable_low[current_wdt_device - 1];
                    iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWRERL);

                    reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWRERH);
                    reg = g_reset_enable_high[current_wdt_device - 1];
                    iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWRERH);

                         
                    /* Set the watchdog timer in running state by setting SYSWCR3[0]=1 */
                    reg = ioread32((void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCR);
                    reg |= PILOT_WATCHDOG_PRETRIGGER_ENABLE | PILOT_WATCHDOG_RESET_ENABLE;
                    iowrite32(reg , (void __iomem*)SE_RES_DEB_VA_BASE + (current_wdt_device - 1)*0x10 + SYSWCR);

                    /* Trigger the watchdog timer by setting SYSWCR3[23]=1. This starts the countdown from the counter value loaded in SYSWCFR3[15:0] register */
