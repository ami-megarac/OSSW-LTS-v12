...
adc_module_ioctlUnlocked(struct file *file, uint cmd, ulong arg)
{
	...
	uint16_t		ref_voltage, resolution, mode;
    ...
    switch (cmd)
	{
		...
		case SET_AUTO_COMPENSATING:

			if (pdev->padc_hal->padc_hal_ops->adc_set_compensating_mode)
			{
				if (copy_from_user (&mode, (void*) arg, sizeof(uint16_t)))
				{
           		  	printk("SET_AUTO_COMPENSATING: Error copying data from user\n");
					return -EFAULT;
				}
				pdev->padc_hal->padc_hal_ops->adc_set_compensating_mode(mode);				
			}
			break;
