...
//		*pDATA32 = cpu_to_le32(Info->Speed);
	...
	case OID_GEN_VENDOR_DESCRIPTION:
		if(AllocCmplt(strlen(Info->VendorDesc)) != NULL)
			memcpy(pDATA8, Info->VendorDesc, strlen(Info->VendorDesc)); /* Fortify [Dangerous Function: strcpy()]:: False Positive *//* Reason for False Positive - Using memcpy() instead of strcpy(). cause of "the terminating null byte" is not needed for response data. */
		break;
...
static int HandleRndisSetMsg(NET_DEVICE *pDev, RNDIS_MSG *pMsg)
{
	...
	case OID_GEN_CURRENT_PACKET_FILTER:
		...
		printk ("Filter = %x\n", (unsigned int)Filter);
		Info->State = (Info->Filter) ? /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - return date type is RNDIS_STATE and it matches as design. */
...
static int HandleRndisCommand(NET_DEVICE *pDev, RNDIS_MSG *pMsg)
{
	...
	case REMOTE_NDIS_SET_MSG:
		TDBG("**************REMOTE_NDIS_SET_MSG**************\n");
		return HandleRndisSetMsg(pDev, pMsg);
		
	case REMOTE_NDIS_HALT_MSG:
		ClearRndisPendingResponse ();
		TDBG("**************REMOTE_NDIS_HALT_MSG ???**************\n");
		Info->State = RNDIS_UNINITIALIZED;
		return 1;
	
	case REMOTE_NDIS_RESET_MSG:
		TDBG("**************REMOTE_NDIS_RESET_MSG ???**************\n");
		ClearRndisPendingResponse ();
		return HandleRndisResetMsg(pDev, pMsg);
		
	case REMOTE_NDIS_KEEPALIVE_MSG:
		TDBG("**************REMOTE_NDIS_KEEPALIVE_MSG ???**************\n");
		return HandleRndisKeepAliveMsg(pDev, pMsg);
		
	case REMOTE_NDIS_INDICATE_STATUS_MSG:
		TDBG("HandleRndisCommand(): Unimplemented MessageType : %d\n", pMsg->MessageType);
		return 1;
		
	default:
		TDBG("HandleRndisCommand(): Unknown MessageType : %d\n", pMsg->MessageType);
		return 1;
	}
...
int HandleRndisRequest(NET_DEVICE *pDev, DEVICE_REQUEST *pRequest)
{
	...
	default:
		TDBG("HandleRndisRequest() : Unknown request : %d\n", pRequest->bRequest);
		ret = 1;
	}
