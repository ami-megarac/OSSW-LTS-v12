...
static void RndisNotifyHost(void)
{
	...
	epnum = NotifyEpNum [0]; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "NotifyEpNum" from int to uint8 to fit return value. */
...
{
	static uint32 Length; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "Length" from static int to static uint32 to fit return value. */
	...
	RxData = Data;
	RxLen = Length; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "Length" from static int to static uint32 to fit return value. */
...
static inline void SendUsbData(uint8 *Data, uint16 Length, uint8 ZeroLenPkt)
{
	...
	// epnum = UsbCore.CoreUsbGetDataInEp(DeviceNumber, EthDataIfNum); /* Fortify [Poor Style: Value Never Read]:: False Positive *//* Marked unneeded statement. value from CoreUsbGetDataInEp() for epnum changed by next if() statement. */
	if(IsRndis())
		epnum = DataInEpNum [0]; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "DataInEpNum" from int to uint8 to fit return value. */
	else
		epnum = DataInEpNum [1]; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "DataInEpNum" from int to uint8 to fit return value. */
...
static int EthNetTxHandler(NET_PACKET *Pkt) // Pkt will be null when called by EthTxHandler
{
	...
	uint16 EpSize; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "EpSize" from int to uint16 to fit return value. */
	int EpNum;
	...
	if(Pkt) // new packet
	{
		...
		if(IsRndis()) // First RNDIS usb packet will contain header
		{
			...
			{
				spin_unlock_irqrestore(&Lock, flags);
				unlocked_flag = 1; /* Fortify [Poor Style: Value Never Read]:: False Positive *//* To avoid dead lock with unpredictable case */
			...
			if(unlocked_flag != 1)
				spin_unlock_irqrestore(&Lock, flags); /* Fortify [Dead Code]:: False Positive *//* To avoid dead lock with unpredictable case */
			return 0;
		}
		if(unlocked_flag != 1)
				spin_unlock_irqrestore(&Lock, flags);
	...
	if((Count = Current->Size - Current->Sent) > EpSize)
		Count = EpSize; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Change the data type of parameter "EpSize" from int to uint16 to fit return value. */
