...
int gpio_poll_ext(struct file* file, poll_table *wait)
{
    int instance;
    pending_gpio_intr_info *node;
    struct list_head *pos;
    instance = ReturnAppInst(current->tgid, current->comm); //Get Instance of the application to use in poll_wait
    if (instance == -1)
        return 0;

    poll_wait(file, &gpio_gpioint_wq[instance], wait);
        ...
        if(tmpnode->pid == current->tgid)
        {
            for(j=0;j<256;j++)
            {
                if(tmpnode->gpio_intr[j].gpio_intr_enabled == 1) 
                {
                    ...
                    {
                        gpin_data.PinNum = (unsigned short)j;
    ...
    switch (cmd)
    {
        case IOCTL_GPIO_INT_REGISTER:
        {
            ...
            if(ReturnAppInst(current->tgid,current->comm) == -1)
            {
                ...
                newnode->pid = current->tgid; //process ID	/* Fortify [Type Mistmatched: signed to unsigned] :: False Positive */
