...
char CommandNum[COMM_MAX_LEN] = {0};
...
Cmd2Str(NCSI_IF_INFO *info)
{
	...
	{
		retval = snprintf(CommandNum, sizeof(CommandNum) ,"0x%02x",info->LastCommand);
		if(retval < 0 || retval >= (int)sizeof(CommandNum))
		{
			printk("Cmd2Str Buffer Overflow\n");			
			return NULL;
		}
		return CommandNum;
	}
