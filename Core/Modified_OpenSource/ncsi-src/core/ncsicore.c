...
extern struct timer_list DetectTimer;  /* A Kernel Timer to change link status */
extern int isDetectTimerInitialized;
...
void ChangeLink ( NCSI_IF_INFO *info )
{
	struct ftgmac100 *priv = netdev_priv(info->dev);
		
	if((priv->use_ncsi == 1) && (InitComplete == 1)){
		info->LinkStatus=0;
...
/*
 * @Brief : Periodically change link status for no detect NCSI and PHY device.
 *          Start when no detect NCSI and PHY device, stop when get NCSI device.
 */
static void
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
DetectTimerFn(unsigned long data)
#else
DetectTimerFn(struct timer_list *timer)
#endif
{
	InvokeCallbackForEachInterface(ChangeLink);

	// Respawning the timer
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
	DetectTimer.expires = jiffies + (10 * HZ);
	add_timer(&DetectTimer);
#else
	mod_timer(&DetectTimer, jiffies + (10 * HZ));
#endif
}
...
void EnableDetectTimer(NCSI_IF_INFO *info)
{
	...
	if (info->TotalChannels == 0){
		if ( !isDetectTimerInitialized && (priv->use_ncsi == 1))
		{
			printk("NCSI(%s): Initializing the timer for change link status.\n", info->dev->name);
			#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
				init_timer(&DetectTimer);
				DetectTimer.data = (unsigned long)info;
				DetectTimer.function = DetectTimerFn;
				DetectTimer.expires = jiffies + (10 * HZ);
				add_timer(&DetectTimer);
			#else
				timer_setup(&DetectTimer, DetectTimerFn, 0);
				mod_timer(&DetectTimer, jiffies + (10 * HZ)); 
			#endif
				isDetectTimerInitialized = 1;
		}
	}else{
		if(isDetectTimerInitialized){
			printk("NCSI(%s): Delete the timer for change link status.\n", info->dev->name);
			del_timer(&DetectTimer);
		}
	}
}
...
//	}
	...
	if (strncmp (dev->name, CONFIG_SPX_FEATURE_NCSI_DEFAULT_INTERFACE, MAX_IF_NAME_LEN) != 0)
        {
                printk("NCSI(%s): Not setting as default interface, skip scanning.\n",dev->name);
                return;
        }
