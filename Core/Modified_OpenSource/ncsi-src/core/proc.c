...
static int proc_reenable(struct ctl_table *ctl,int write,void *buffer,size_t *lenp,loff_t* pos);
static int proc_setmac(struct ctl_table *ctl,int write,void *buffer,size_t *lenp,loff_t* pos);
#else
...
static int proc_initcomplete(struct ctl_table *ctl,int write,struct file *filep,void *buffer,size_t *lenp,
        					loff_t* pos);
static int proc_reenable(struct ctl_table *ctl,int write,struct file *filep,void *buffer,size_t *lenp,
        					loff_t* pos);
static int proc_setmac(struct ctl_table *ctl,int write,struct file *filep,void *buffer,size_t *lenp,
                            loff_t* pos);  
#endif
...
static struct ctl_table SysNcsiTable[] = 
{
    ...
    {.procname="ReEnable"   ,  		.data=&ReEnable,    		.maxlen=sizeof(int), .mode=0644, .proc_handler=&proc_reenable },
    {.procname="SetMAC"   ,  		.data=&UserSetMAC,    		.maxlen=sizeof(int), .mode=0644, .proc_handler=&proc_setmac },
#ifdef CONFIG_SPX_FEATURE_NCSI_RESET_INTERFACE_IN_USER_SETTINGS
	...
	{"ReEnable"   ,  &ReEnable,    sizeof(int), 0444, NULL, NULL, &proc_reenable },
	{"SetMAC"   ,  &SetMAC,    sizeof(int), 0444, NULL, NULL, &proc_setmac },
#ifdef CONFIG_SPX_FEATURE_NCSI_RESET_INTERFACE_IN_USER_SETTINGS
...
int
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,4,11))
proc_reenable(struct ctl_table *ctl,int write,void *buffer,size_t *lenp, loff_t *pos)
#else
proc_reenable(struct ctl_table *ctl,int write,struct file *filep,void *buffer,size_t *lenp, loff_t *pos)
#endif
{	
	int retval = 0;
	...
	// if ReEnable == 0, do nothing
	if(ReEnable == 0)
		return retval;
	
	if(ReEnable != 1)
	{
		printk(KERN_DEBUG "NCSI: Sysctl \"Reconnect\" should be 1 or 0 \n");
		return retval;
	}
...
int 
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,4,11))
proc_setmac(struct ctl_table *ctl,int write,void *buffer,size_t *lenp, loff_t *pos)
#else
proc_setmac(struct ctl_table *ctl,int write,struct file *filep,void *buffer,size_t *lenp, loff_t *pos)
#endif
{
	int retval = 0;
	#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,4,11))  
	...
	// if UserSetMAC == 0, do nothing
	if (UserSetMAC == 0)
		return retval;
	
	if (UserSetMAC != 1)
	{
		printk(KERN_DEBUG "NCSI: Sysctl \"SetMAC\" should be 1 or 0 \n");
		return retval;
	}
