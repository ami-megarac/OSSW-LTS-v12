...
int ast_usbhub_module_init(void)
{
	...
	get_usb_core_funcs(&usb_core_module); /* Remove check statement for fortify [Dead Code].*/
	...
	IRQ_SET_LEVEL_TRIGGER(hub_irq);
	IRQ_SET_HIGH_LEVEL(hub_irq);	
...
#endif
	if (ret != 0) {
		printk(KERN_ERR "%s: Request IRQ %d failed.\n", ast_usbhub_driver_name, hub_irq);
		goto out_iomap;
	}
