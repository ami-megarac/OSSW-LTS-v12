			...
			char *buffer, size_t buflen, int *errnop)
{
	...
	if(CheckBufferOverflow(nss_res.usr.name,sizeof(nss_res.usr.name)))
	{
		TCRIT("Buffer Overflow\n");
		result = NSS_STATUS_UNAVAIL;
		return result;
	}
	ADD_STRTOBUF_REF(pwd->pw_name, nss_res.usr.name, init_buf, buffer, buflen);	/* Fortify [String Termination Error]:: False Positive */
	/* Reason for False Positive - Checked the NULL termination of buffer nss_res.usr.name using CheckBufferOverflow() and In macro expanssion using the destination size to fill the destination and also comparing the length of source with destination before writing into the destination */
	...
	{
		ADD_STRTOBUF_REF(pwd->pw_shell, defshell, init_buf, buffer, buflen); /*Fortify[Buffer Overflow] :: False Positive*/
		/* Reason for False Positive - In macro expanssion using the destination size to fill the destination and also comparing the length of source with destination 
		 before writing into the destination and checking the buffer overflow condition for the same */
	}
