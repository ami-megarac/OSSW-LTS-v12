...
INIHandler*  iniparser_loaddef(const char * def_ininame, const char * ininame)
{
    ...
    if(ininame != NULL)
        ini = fopen(ininame, "r") ; /* Fortify [Path Manipulation]:: False Positive */
    /* Reason for False Positive - not accepting user inputs to string def_ininame, updated in source and not using invalid data while using the file */
...
void iniparser_setstring(INIHandler *handler, char *hashstr, char * val)
{
	...
	/*for eg: set username or Domain name in RIS configuration command in which max bytes is 256*/
        ...
        if (val) {
            retval = SNPRINTF(writeVal, sizeof(writeVal),"%s",val); /* Fortify [String Termination Error]:: False Positive */
	    /* Reason for False Positive - Destination is filled as per the destination size and snprintf() will NULL terminate the string, also check for bufferoverflow. */
	...
	/* Reason for False Positive - Destination hashTable has sufficient memeory to store the source data pnvp. */
        return;
    }
    else{
        ...
        while(pnvp) {
            if(pnvp->key) {
                if(strcmp(pnvp->key, hashstr) == 0) {
                    ...
                    if(pnvp->val) {
                    	if ((val == NULL) || (strlen(pnvp->val) != strlen(writeVal))) { /* Fortify [String Termination Error]:: False Positive */ /* use local copy of val in place of external pointer */
				/* Reason for False Positive - Both pnvp->val and writeVal are having NULL terminated String. */
							free(pnvp->val);
                                                        pnvp->val = val ? strdup(writeVal) : NULL; /* use local copy of val in place of external pointer */
                    	} else {
                    		// copy the new value to be sure that the correct value is stored
                                        strcpy(pnvp->val, writeVal);	/* Fortify [Buffer overflow]:: False Positive */ /* Fortify [String Termination Error]:: False Positive */ /* use local copy of val in place of external pointer */
                                        /* Reason for False Positive - destination has sufficient memory to store the source data */
					/* Reason for False Positive - writeVal have NULL terminated String. */
                    	}
                    } else {
                        pnvp->val = val ? strdup(writeVal) : NULL; /* use local copy of val in place of external pointer */
                    }
...
void iniparser_dump_ini(INIHandler *handler, FILE *file)
{
    ...
    if (nsec<1) {
        for (i=0 ; i < HASH_TABLE_SIZE ; i++) {
            ...
            nvp_ptr = hashTable[i];
            while(nvp_ptr != NULL) {
                fprintf(file, "%s=%s\n", nvp_ptr->key, nvp_ptr->val); /*Fortify[Privacy Violation] :: false positive*/
                /* Reason for False Positive - writing the data into the local file sytem only and not using sensitive data without encryption */
                nvp_ptr = nvp_ptr->next;
            }
        }
        return ;
            ...
            while(nvp_ptr != NULL && nvp_ptr->key != NULL) {
                if (!strncmp(nvp_ptr->key, keym, seclen + 1)) {
                    fprintf(file, "%s=%s\n",
                        ...
                        nvp_ptr->val ? nvp_ptr->val : ""); /*Fortify[Privacy Violation] :: false positive*/
                    /* Reason for False Positive - writing the data into the local file sytem only and not using sensitive data without encryption */
                }
                nvp_ptr = nvp_ptr->next;
            }
...
int iniparser_dump_file(INIHandler *handle, char *filename)
{
	...
	// MIND the dependency about cross platform development.
    struct stat pri;
    if( 0 != stat(filename, &pri) ){
        return -1;     
    }
#endif
    /* Save the INI in a backup file. So during failure (poweroff/reset) 
       during write will not corrupt original file */
    retval = SNPRINTF(bkp_filename,sizeof(bkp_filename),"%s%s",filename,".tmp"); /* Fortify [String Termination Error]:: False Positive */
    /* Reason for False Positive - Destination is filled as per the destination size and snprintf() will NULL terminate the string, also check for bufferoverflow. */ 
    if((retval < 0) || (retval >= (signed)sizeof(bkp_filename)))
    {
	printf("Buffer Overflow in File:%s Line: %d  Function : %s\n",__FILE__, __LINE__, FUNCTION_NAME);
	return -1;
    }
    fp = fopen(bkp_filename,"w"); /* Fortify [Path Manipulation]:: False Positive */
...
#if !defined (_WIN32) && !defined (WIN64)
	// MIND the dependency about cross platform development.
    if( 0 != chmod(bkp_filename,pri.st_mode)){ /* Fortify [Path Manipulation]:: False Positive */ /* Fortify [Race Condition: File System Access]:: False Positive */
    /* Reason for False Positive - not accepting user inputs and not using invalid data while using the file */ 
    /* Reason for False Positive - The buffer bkp_filename does not change between the two calls fopen() and chmod(). */
        return -1;
    }
#endif
    /* Now rename backup file to original file - This is a atmoic
	   ...
	   contents */
    if ( -1 == rename(bkp_filename,filename) ) /* Fortify [Path Manipulation]:: False Positive */
    /* Reason for False Positive - not accepting user inputs  to strings bkp_filename and filename, updated in source and not using invalid data while using the file */
