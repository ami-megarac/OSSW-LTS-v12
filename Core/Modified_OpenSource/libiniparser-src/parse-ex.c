...
{
    char Str[MAX_STRSIZE] = {0};
    int ret = 0;
    
    ret = SNPRINTF(Str,sizeof(Str),"%lu",Val);	/* Fortify [String Termination Error]:: False Positive */
    /* Reason for False Positve - the number of bytes writen into the destination is less than the size of destination and destination is NULL terminated */
    if(ret < 0 || ret >= (signed)sizeof(Str))
    {
        printf("Buffer Overflow in File:%s Line: %d  Function : %s\n",__FILE__, __LINE__, FUNCTION_NAME);
        return;
    }
...
IniSetSInt(INI_HANDLE handle, char *Section, char *Key, long Val)
{
    ...
    ret = SNPRINTF(Str,sizeof(Str), "%ld",Val);	/* Fortify [String Termination Error]:: False Positive */
    /* Reason for False Positve - the number of bytes writen into the destination is less than the size of destination and destination is NULL terminated */
	if(ret < 0 || ret >= (signed)sizeof(Str))
	{
		printf("Buffer Overflow in File:%s Line: %d  Function : %s\n",__FILE__, __LINE__, FUNCTION_NAME);
...
IniSaveFile(INI_HANDLE handle,char *filename)
{
	...
	/* Now rename backup file to original file - This is a atmoic
	   ...
	   contents */
	if ( -1 == rename(bkp_filename,filename) )	/* Fortify [Race Condition: File System Access]:: False Positive */
		/* Reason for False Positve - the string bkp_filename doesn't change between the calls of fopen() and rename() */
